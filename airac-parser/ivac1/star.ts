...
export const extractSTAR = async (db: Promise<Database>, airport: {airport_id: number; ident: string}) => {
  let out = '';
  // Query for STARs
    ...
    for (const star_id of star_ids) {
      ...
      const legs = await (await db).all<{
        ...
        WHERE
          approach_id = ${star_id}
      `);
      const points: string[] = await legsToPoints(legs, airport.airport_id, (await db));
      ...
      for (const star_transition_id of star_transition_ids) {
        ...
        const transition_name = `${airport.ident}-${star.arinc_name} ${
          star.fix_ident

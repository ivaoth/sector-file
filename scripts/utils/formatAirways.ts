...
export const formatAirways = (airways: Segment[][], mode: "J" | "V") => {
  let out = "";
  ...
  for (const label of segmentLabelMap) {
    out += `L;${label.airways.map(s => `${s.name}${s.direction}`).join("/")};${(
      (label.from_lat + label.to_lat) /
      2
    ).toFixed(6)};${((label.from_lon + label.to_lon) / 2).toFixed(6)};\n`;
  }
  return out;
};
